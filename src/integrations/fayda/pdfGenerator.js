...
const TEMPLATE_PATH = path.join(__dirname, "assets", "raw", "assets_images_efaydapdftemplate.pdf");
const ENGLISH_FONT_PATH = path.join(__dirname, "assets", "raw", "assets_fonts_barlowsemicondensedmedium.ttf");
const AMHARIC_FONT_PATH = path.join(__dirname, "assets", "raw", "assets_fonts_nyala.ttf");
...
function normalizeBase64(value) {
  ...
  const stripped = String(value).trim().replace(/^data:[^;]+;base64,/, "").replace(/\s+/g, "");
...
function sanitizeVerifyResponse(response) {
  ...
  const data = cleanResponse?.user?.data || cleanResponse?.data || {};
...
async function generateDigitalIdPdf(verifyResponse, outputPath) {
  const { cleanResponse, pdfData } = sanitizeVerifyResponse(verifyResponse);
  const [templateBytes, englishFontBytes, amharicFontBytes] = await Promise.all([
    fs.readFile(TEMPLATE_PATH),
    fs.readFile(ENGLISH_FONT_PATH),
    fs.readFile(AMHARIC_FONT_PATH)
  ]);
  ...
  // Persist to disk ONLY if a path was explicitly requested. The merged service
  // streams the buffer to the response and never writes a file.
  if (outputPath) {
